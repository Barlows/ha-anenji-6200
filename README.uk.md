...
базові показання, а на пристроях із відповідними відповідями — також дані BMS
та номінальні параметри. Це визначення протоколу, не комерційної моделі.
Додаткові сутності вимкнені за замовчуванням. Дивіться
...
Зберігайте резервні копії **поза** `config/custom_components/`. Перейменування
старої папки на `eybond_local_backup` усередині цього каталогу не вимикає її:
Home Assistant може знайти незмінений manifest і завантажити старий код.
Залиште там лише потрібну папку `eybond_local/` і повністю перезапустіть Home
Assistant; після заміни Python-файлів перезавантаження лише інтеграції недостатньо.
